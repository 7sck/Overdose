...
void MonNew()
{
    ...
    for (DWORD pid : pids)
    {
        if (g_rbxHandles.contains(pid))
            continue;

        HANDLE hProc = OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_SET_QUOTA | PROCESS_SET_INFORMATION, FALSE, pid);
        if (!hProc)
        {
            std::cerr << "[Overdose] Failed to open PID " << pid << std::endl;
            continue;
        }

        g_rbxHandles[pid] = hProc;
        std::cout << "[Overdose] New Roblox instance PID " << pid << " hooked." << std::endl;
                    ...
                    do {
                        if (_wcsicmp(crash.szExeFile, L"RobloxCrashHandler.exe") == 0)
                        {
                            ...
                            if (hCrashProc)
                            {
                                ...
                                std::cout << "[Overdose] Killed CrashHandler PID " << crash.th32ProcessID << std::endl;
                            }
                        }
        ...
        std::thread([pid, hProc]()
                    {
                        if (StartTrimmer(hProc))
                            std::cout << "[Overdose] Trimmer started for PID " << pid << std::endl;
                        else
                            std::cerr << "[Overdose] Trimmer failed for PID " << pid << std::endl;

                        if (ApplyCPULimits(hProc))
                            std::cout << "[Overdose] CPU limits applied for PID " << pid << std::endl;
                        else
                            std::cerr << "[Overdose] CPU limit failed for PID " << pid << std::endl; })
            .detach();
    }
}
...
void CleanupExited()
{
    ...
    for (auto it = g_rbxHandles.begin(); it != g_rbxHandles.end();)
    {
        ...
        {
            std::cout << "[Overdose] Roblox PID " << it->first << " exited." << std::endl;
...
int main()
{
    ...
    std::cout << "Overdose watching for Roblox..." << std::endl;
...

{
    if (!_wmimon())
    {
        MessageBoxW(nullptr, L"Failed to initialize WMI monitor.", L"Overdose", MB_ICONERROR);
        return -1;
    }
